...
engine = create_engine("mysql+pymysql://root:root@localhost/mysql")
...
def register():
    if request.method == "POST":
        ...
        username_data = db.execute(
            text("SELECT username FROM users WHERE username=:username"), {"username": username}).fetchone()
        usernamedata = db.execute(
            "SELECT username FROM users WHERE username=:username", {"username": username}).fetchone()
        ...
        if usernamedata is None:
            ...
            for passwor_data in passworddata:
                if sha256_crypt.verify(password, passwor_data):
                    ...
                    return redirect(url_for('home'))  # to be edited from here do redirect to either svm or home
                else:
